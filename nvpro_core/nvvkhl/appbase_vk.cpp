...
void nvvkhl::AppBaseVk::submitFrame()
{
  ...
  setViewportLE(m_commandBuffers[imageIndex]);
  // render scene
  setViewportRE(m_commandBuffers[imageIndex]);
  // render scene
...
void nvvkhl::AppBaseVk::setViewportLE(const VkCommandBuffer& cmdBuf)
{
  float width  = static_cast<float>(m_size.width) / 2.0f;  // Half width for each camera
  float height = static_cast<float>(m_size.height);
...
void nvvkhl::AppBaseVk::setViewportRE(const VkCommandBuffer& cmdBuf)
{
  float width  = static_cast<float>(m_size.width) / 2.0f;  // Half width for each camera
  float height = static_cast<float>(m_size.height);

  VkViewport viewport{width, 0.0f, width, height, 0.0f, 1.0f};
  vkCmdSetViewport(cmdBuf, 0, 1, &viewport);
