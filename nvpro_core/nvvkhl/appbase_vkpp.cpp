...
void nvvkhl::AppBase::createFrameBuffers()
{
  ...
  framebufferCreateInfo.width        = m_size.width / m_cameraCount;
  ...
  m_framebuffers.resize(m_swapChain.getImageCount() * m_cameraCount);
  for(uint32_t i = 0; i < m_swapChain.getImageCount(); i++)
  {
    ...
    // left eye
    m_framebuffers[i * m_cameraCount] = m_device.createFramebuffer(framebufferCreateInfo);
    // right eye
    m_framebuffers[i * m_cameraCount + 1] = m_device.createFramebuffer(framebufferCreateInfo);
  }
